/**
 * Ported over from https://github.com/microsoft/vscode-markdown-it-katex
 * to fix the CJS issue. Should be inlcuded as a dependency if the issue if fixed.
 */
...
import type { MarkdownKatexOptions } from "./types";
...
function isValidInlineDelim(
	...
	if (
		prevChar !== "$" && prevChar !== "\\" && (
			prevChar === undefined || isWhitespace(prevChar)
...
function blockBareMath(
	...
	const token = state.push("math_block", "math", 0);
	token.block = true;
	token.content = (state.getLines(start, next, state.tShift[start], true)
		+ (lastLine ?? "")).trim();
	token.map = [start, state.line];
	token.markup = "$$";
	return true;
...
function inlineBareBlock(state: StateInline, silent: boolean): boolean {
	...
	const endIndex = lines.slice(0, foundLine + 1).reduce((p, c) => p + c.length, 0)
		+ foundLine + 1;
...
export default function(
	...
	const katexInline = (latex: string) => {
		const displayMode = /\\begin\{(align|equation|gather|cd|alignat)\}/ig
			.test(latex);
		try {
	...
	const inlineRenderer = (tokens: readonly Token[], idx: number) => {
		...
		const hasBacktick = content.length > 2 && content[0] === "`"
	...
	const katexBlockRenderer = (latex: string) => {
		try {
			return `<p class="katex-block">${
				katex.renderToString(latex, { ...options, displayMode: true })
			}</p>`;
		} catch (error) {
			if (options?.throwOnError) {
				console.log(error);
	...
	if (enableFencedBlocks) {
		...
		md.renderer.rules.fence = function(
			...
			if (
				...
				return originalFenceRenderer?.call(
					this,
					tokens,
					idx,
					options,
