...
export const markdownItFancyListPlugin = (
	markdownIt: MarkdownIt,
	options?: MarkdownItFancyListPluginOptions,
): void => {
	...
	function parseOrderedListMarker(
		...
		const stringContainingNumberAndMarker = state.src.substring(
			start,
			Math.min(max, start + 10),
		);

		const match =
			/^(\d{1,9}|[a-z]{1,3}|[A-Z]{1,3}|[ivxlcdm]+|[IVXLCDM]+|#)([\u00BA\u00B0\u02DA\u1D52]?)([.)])/
				.exec(stringContainingNumberAndMarker);
		if (match === null) {
			return null;
		}
		...
		if (
			isCharCodeUppercaseAlpha(match[1].charCodeAt(0)) && match[1].length === 1
			&& markerChar === "."
		) {
	...
	const convertAlphaMarkerToOrdinalNumber = (alphaMarker: string): number => {
		const lastLetterValue = alphaMarker.toLowerCase().charCodeAt(alphaMarker.length - 1)
			- "a".charCodeAt(0) + 1;
	...
	function analyseMarker(
		...
		if (orderedListMarker !== null) {
			...
			if (isCharCodeDigit(charCode)) {
				...
				const preferRoman = (previousMarker !== null && previousMarker.isRoman === true)
					|| ((previousMarker === null || previousMarker.isAlpha === false)
						&& (bulletChar === "i" || bulletChar.length > 1));
	...
	const createFancyList = (options: MarkdownItFancyListPluginOptions) => {
		return (
			...
			while (nextLine < endLine) {
				...
				const initial = state.sCount[nextLine] + pos
